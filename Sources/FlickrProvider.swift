...
class FlickrProvider {
    ...
    class func fetchPhotosForSearchText(searchText: String,page:Int=1, onCompletion: @escaping FlickrResponse) -> Void {
        let escapedSearchText: String = searchText.addingPercentEncoding(withAllowedCharacters:.urlHostAllowed)!
        let urlString: String = "https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=\(Keys.flickrKey)&text=\(escapedSearchText)&per_page=500&page=\(page)&format=json&nojsoncallback=1"
